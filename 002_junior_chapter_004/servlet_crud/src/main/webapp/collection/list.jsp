...
<form action="<%=request.getContextPath()%>/collection/create" method="get">
    <p align="center"><input type="submit" value="create user"></p>
</form>

<div align="center">
    <h1>Users list: collection</h1>
</div>
...
<table style="border: 1px solid black;" cellpadding="5px" cellspacing="0px" border="1px" align="center" valign="center">
    ...
    <% for (User user : UserValidatorInCollection.getInstance().findAll()) { %>
    <tr align="center" valign="center">
        <td><%=user.getId()%>
        </td>
        <td><%=user.getName()%>
        ...
        <td>
            <form action="<%=request.getContextPath()%>/collection/update" method="get">
                <input type="hidden" name="id" value="<%=Integer.toString(user.getId())%>">
                <input type="submit" value="update">
            </form>
        </td>
        <td>
            <form action="<%=request.getContextPath()%>/collection" method="post">
                <input type="hidden" name="id" value="<%=Integer.toString(user.getId())%>">
