...
import static ru.job4j.streams.exchange.ActionEnum.ADD;
import static ru.job4j.streams.exchange.ActionEnum.DELETE;
import static ru.job4j.streams.exchange.OperationEnum.ASK;
import static ru.job4j.streams.exchange.OperationEnum.BID;
...
class OrderBook {
    ...
    private Pair<Task, Set<Task>> findTaskAndItsSetByIdInOneMap(Map<Integer, Set<Task>> map, String id) {
        Task taskFound = map.values().stream()
                .flatMap(Collection::stream)
                .filter(task -> id.equals(task.id()))
                .findFirst()
                .orElse(null);
        Set<Task> setFound = taskFound != null
                ? map.get(taskFound.price())
                : null;
        return new Pair<>(taskFound, setFound);
    }
    ...
    public String toString() {
        ...
        this.addToBufferIfNeeded(buffer, sellTasks);
        this.addToBufferIfNeeded(buffer, buyTasks);
        return buffer.toString();
    }

    /**
     * Adds output string to buffer if string is not empty.
     *
     * @param buffer   Buffer to add string to.
     * @param addition String.
     */
    private void addToBufferIfNeeded(StringJoiner buffer, String addition) {
        if (!"".equals(addition)) {
            buffer.add(addition);
        }
    }
    ...
    private String toStringTasksFromMap(String format, Map<Integer, Set<Task>> map, boolean volumeLeftSide) {
        StringJoiner buffer = new StringJoiner(System.lineSeparator());
        map.forEach((price, tasks) -> {
            int volume = tasks.stream()
                    .map(Task::volume)
